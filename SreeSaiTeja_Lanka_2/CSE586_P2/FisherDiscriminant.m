...
MdlLinear = Fisher_linear_disc(train_featureVector, classes_count, pre_train_labels);
...
%      features)
[a,b]  = size(feature_idx);
k = round(sqrt(b));
train_predict = KNN(MdlLinear, train_featureVector, train_featureVector, k, pre_train_labels);
...
%      function (the function for finding the class labels from a set of
%      features)
test_predict = KNN(MdlLinear, train_featureVector, test_featureVector, k, pre_train_labels);
